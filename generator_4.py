...
def writeUniquePayload(code,l_encrypted,decryptKey,decryptIV,outfile=payload2):
    ...
    # c = code.splitlines()
    for line in code:
        print red("DEBUG: Type(code):\r\n{}".format(str(type(code))))
        print red("DEBUG: Type(line):\r\n{}".format(str(type(line))))
        if 'key=' in line:
...
#         # convert back into strings from list objects
#         shuffledLine = shuffledPayload[k][0]
#         arrayMap = shuffledPayload[k][1]
#         sL = str("-".join(shuffledLine))
#         # convert all integers into strings
#         arrayMap = map(str,arrayMap)
#         aM = str("-".join(arrayMap))
#         print red("DEBUG: Recombined string shuffledLine\r\n{}".format(str(sL)))
#         print red("DEBUG: Recombined arrayMap\r\n{}".format(str(aM)))
#         writableLine = '{}|{}\r\n'.format(str(sL),str(aM))
